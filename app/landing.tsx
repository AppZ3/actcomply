...
// Kept in step with REQUIREMENTS_MAPPED in lib/eu-ai-act.ts. Only the seed for
// the first paint; /api/stats replaces it on mount.
const REQUIREMENTS_MAPPED_FALLBACK = 27
...
function nextMilestoneFallback(): NextMilestone | null {
  const now = new Date()
  const next = MILESTONE_FALLBACK.find(m => new Date(m.iso).getTime() > now.getTime())
  if (!next) return null
  // Measured from UTC midnight, matching getEnforcementStatus, so this renders
  // identically on the server and at hydration.
  const today = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate())
  return {
    key: next.key,
    label: next.label,
    displayDate: next.displayDate,
    daysUntil: Math.ceil((new Date(next.iso).getTime() - today) / 86_400_000),
  }
}
...
export function LandingPage() {
  ...
  const [nextMilestone, setNextMilestone] = useState<NextMilestone | null>(nextMilestoneFallback)
  const [requirementsMapped, setRequirementsMapped] = useState<number | null>(REQUIREMENTS_MAPPED_FALLBACK)
      ...
      <section className="border-y border-gray-200 dark:border-white/10 py-12 bg-white dark:bg-transparent">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            { value: `${requirementsMapped ?? REQUIREMENTS_MAPPED_FALLBACK}+`, label: 'Compliance requirements mapped', live: true },
          ].map(stat => (
            <div key={stat.label}>
