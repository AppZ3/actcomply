...
const GDPR_STEPS = [
  ...
  'Writing DPIA findings',
  'Assessing fundamental rights impacts (FRIA)',
  'Writing Article 86 explainability statement',
]
...
function GeneratingProgress() {
  ...
  const allDone = elapsed >= GDPR_STEPS.length * 10

  useEffect(() => {
    setActiveStep(Math.min(Math.floor(elapsed / 10), GDPR_STEPS.length - 1))
  }, [elapsed])

  return (
    <div className="mt-5 space-y-3">
      <div className="flex items-center justify-between text-xs text-gray-500">
        <span>{allDone ? 'Finalising and saving…' : 'Claude is generating your DPIA + FRIA…'}</span>
        <span className="font-mono tabular-nums">{elapsed}s</span>
      </div>
      <div className="space-y-1.5">
        {GDPR_STEPS.map((name, i) => {
