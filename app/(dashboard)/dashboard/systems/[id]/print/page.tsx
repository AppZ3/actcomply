...
export default async function PrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ActComply mark.
  let branding: { name: string; logo_url: string | null; brand_color: string } | null = null
  if (planFeatures.whiteLabel && assessment.org_id) {
    const { data: org } = await admin
      .from('organizations')
      .select('name, brand_name, logo_url, brand_color')
    ...
    if (org) {
      branding = {
        ...
        brand_color: org.brand_color || '#0f172a',
      }
    }
  }
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-10 py-12 bg-white text-gray-900 min-h-screen print:p-0">
              ...
              {branding ? (
                <div className="flex items-center gap-3 mb-2">
                  {branding.logo_url && (
                    ...
                    <img src={branding.logo_url} alt={`${branding.name} logo`} className="h-10 w-auto object-contain" />
                  )}
                  <div className="text-xs font-semibold uppercase tracking-widest" style={{ color: branding.brand_color }}>
                    {branding.name} · EU AI Act Compliance Report
                  </div>
                </div>
              ) : !planFeatures.whiteLabel ? (
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-2">ActComply · EU AI Act Compliance Report</div>
              ) : null}
        ...
        <div className="border-t border-gray-200 pt-6 text-xs text-gray-400 text-center break-inside-avoid">
          {branding
            ? `${branding.name} · This document is for compliance guidance only and does not constitute legal advice.`
            : planFeatures.whiteLabel
